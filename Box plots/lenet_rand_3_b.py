...
for r in range(number_of_runs):
	...
	model.add(Conv2D(4, kernel_size=(5, 5),
		         activation='relu',
		         input_shape=input_shape,kernel_initializer='truncated_normal',kernel_regularizer=regularizers.l2(0.0)))
	model.add(MaxPooling2D(pool_size=(2, 2)))
	model.add(Conv2D(6, (5, 5), activation='relu',kernel_initializer='truncated_normal',kernel_regularizer=regularizers.l2(0.0)))
	model.add(MaxPooling2D(pool_size=(2, 2)))
	model.add(Flatten())
	model.add(Dense(100, activation='relu',kernel_initializer='truncated_normal',kernel_regularizer=regularizers.l2(0.0)))
	#model.add(Dropout(0.5))
	model.add(Dense(num_classes, activation='softmax',kernel_initializer='truncated_normal',kernel_regularizer=regularizers.l2(0.0)))
	
	# Freeze(not included in training/weights not updated) specific number of layers in the model
	# model.layers = [Conv,Pool,Conv,Pool,Flatten,Dense,Dropout(removed),Dense]
